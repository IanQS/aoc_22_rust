...
// Too lazy to use Range. Just doing it with raw values
pub fn problem2(f_name: &str) -> usize {
    let split_values = read_file(f_name);
    ...
    let filtered_results: Vec<bool> = split_values.iter()
        ...
        .map(|line| {
            ...
            let superset: bool = (s1 <= s2 && e2 <= e1) ||
                (s2 <= s1&& e1 <= e2);

            let intersect: bool = (s1 <= s2 && s2 <= e1) ||
                (s1 <= e2 && e2 <= e1) ||
                (s2 <= s1 && s1 <= e2) ||
                (s2 <= e1 && e1 <= e2);

            return superset || intersect;
        })
        .filter(|x| *x)
        .collect()
        ;
